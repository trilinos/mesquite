...
/** \file TRel3DMetricTest.cpp
 *  \brief Unit tests TRel3DMetric base class functionality
...
// Test functions implemented in class TRel3DMetric
class TRel3DMetricTest : public CppUnit::TestFixture
{
  CPPUNIT_TEST_SUITE( TRel3DMetricTest );
...
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION( TRel3DMetricTest, "Unit" );
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION( TRel3DMetricTest, "TRel3DMetricTest" );

// implement metric such that the expected derivatives
// at each location r,c in dm/dT are 3r+c+1
class GradTestMetricRel3D : public TRel3DMetric
{
  public:
...
class HessTestMetricRel3D : public TRel3DMetric
{
  public:
 ...
 */
class HessTestMetricRel3D_2 : public TRel3DMetric
{
  public:
...
void TRel3DMetricTest::test_numerical_gradient()
{
  GradTestMetricRel3D metric;
  HessTestMetricRel3D_2 metric2;
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel3DMetric::evaluate_with_grad( A, gval, d, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, gval, 1e-6 );
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel3DMetric::evaluate_with_grad( B, gval, d, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, gval, 1e-6 );
  ASSERT_MATRICES_EQUAL( da, d, 1e-6 );
...
void TRel3DMetricTest::test_numerical_hessian()
{
  HessTestMetricRel3D metric;
  HessTestMetricRel3D_2 metric2;
  const double Avals[] = { 1, 2, 3, 4, 1, 4, 3, 2, 1 };
  const double Bvals[] = { 0.1, 0.15, 0.05, 0.2, -0.1, -0.15, -0.05, -0.2, 2 };
  const MsqMatrix<3,3> A( Avals );
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel3DMetric::evaluate_with_hess( A, hval, gh, h, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, hval, 1e-6 );
  ASSERT_MATRICES_EQUAL( g, gh, 1e-6 );
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel3DMetric::evaluate_with_hess( B, hval, gh, h, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, hval, 1e-6 );
  ASSERT_MATRICES_EQUAL( g, gh, 1e-6 );
