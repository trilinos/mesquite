...
/** \file TRel2DMetricTest.cpp
 *  \brief Unit tests TRel2DMetric base class functionality
...
class TRel2DMetricTest : public CppUnit::TestFixture
{
  CPPUNIT_TEST_SUITE( TRel2DMetricTest );
...
// at each location r,c in dm/dT are 2r+c+1
class GradTestMetricRel2D : public TRel2DMetric
{
  public:
...
// | 0 -8 10  0 |
// |_0  0  0  2_|
class HessTestMetricRel2D : public TRel2DMetric
{
  public:
 ...
 */
class HessTestMetricRel2D_2 : public TRel2DMetric
{
  public:
...
void TRel2DMetricTest::test_numerical_gradient()
{
  GradTestMetricRel2D metric;
  HessTestMetricRel2D_2 metric2;
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel2DMetric::evaluate_with_grad( A, gval, d, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, gval, 1e-6 );
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel2DMetric::evaluate_with_grad( B, gval, d, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, gval, 1e-6 );
  ASSERT_MATRICES_EQUAL( da, d, 1e-6 );
...
void TRel2DMetricTest::test_numerical_hessian()
{
  HessTestMetricRel2D metric;
  HessTestMetricRel2D_2 metric2;
  const double Avals[] = { 1, 2, 2, 5 };
  const double Bvals[] = { -0.1, -0.15, -0.25, -0.8 };
  const MsqMatrix<2,2> A( Avals );
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel2DMetric::evaluate_with_hess( A, hval, gh, h, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, hval, 1e-6 );
  ASSERT_MATRICES_EQUAL( g, gh, 1e-6 );
  ...
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  valid = metric2.TRel2DMetric::evaluate_with_hess( B, hval, gh, h, err );
  ASSERT_NO_ERROR(err);
  CPPUNIT_ASSERT(valid);
  CPPUNIT_ASSERT_DOUBLES_EQUAL( val, hval, 1e-6 );
  ASSERT_MATRICES_EQUAL( g, gh, 1e-6 );
...
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION( TRel2DMetricTest, "Unit" );
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION( TRel2DMetricTest, "Target2DTest" );
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION( TRel2DMetricTest, "TRel2DMetricTest" );
