...
class TargetCalculator3D : public TargetCalculator
{
  ...
  public:
    ...
    virtual bool get_3D_target( PatchData&, size_t, const SamplePoints*, unsigned, MsqMatrix<3,3>& result, MsqError& )
      { result = target; return true; }
    
    virtual bool get_2D_target( PatchData&, size_t, const SamplePoints*, unsigned, MsqMatrix<3,2>& result, MsqError& )
...
void LVQDTargetTest::test_product_2D()
{
  ...
  rval = lvqd1.get_2D_target( pd, 0, 0, 0, W, err );
  ...
  rval = lvqd2.get_2D_target( pd, 0, 0, 0, W, err );
  CPPUNIT_ASSERT(!MSQ_CHKERR(err));
  CPPUNIT_ASSERT(rval);
...
void LVQDTargetTest::test_product_3D()
{
  ...
  LVQDTargetCalculator lvqd1( &tc1, &tc1, &tc1, &tc1 );
  rval = lvqd1.get_3D_target( pd, 0, 0, 0, W, err );
  CPPUNIT_ASSERT(!MSQ_CHKERR(err));
  CPPUNIT_ASSERT(rval);
  ASSERT_MATRICES_EQUAL( t1, W, 1e-6 );
  ...
  LVQDTargetCalculator lvqd2( &tc2, &tc2, &tc2, &tc2 );
  rval = lvqd2.get_3D_target( pd, 0, 0, 0, W, err );
  CPPUNIT_ASSERT(!MSQ_CHKERR(err));
  CPPUNIT_ASSERT(rval);
  ASSERT_MATRICES_EQUAL( t2, W, 1e-6 );
}
