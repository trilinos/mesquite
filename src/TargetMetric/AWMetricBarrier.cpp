...
do_finite_difference( int r, int c, AWMetricBarrier* metric, 
                      MsqMatrix<Dim, Dim> A, 
                      ...
                      double value, MsqError& err )
{
  ...
  for (double step = INITAL_STEP; step > std::numeric_limits<double>::epsilon(); step *= 0.1) {
    A(r,c) = init + step;
    valid = metric->evaluate( A, W, diff_value, barrier_violated, err ); MSQ_ERRZERO(err);
    if (valid)
...
template <unsigned Dim>
static inline bool
do_numerical_gradient( AWMetricBarrier* mu,
                       MsqMatrix<Dim, Dim> A,
                       const MsqMatrix<Dim, Dim>& W,
                       double& result,
...
{
  bool barrier_violated = false;
  bool valid = mu->evaluate( A, W, result, barrier_violated, err );
...
template <unsigned Dim>
static inline bool
do_numerical_hessian( AWMetricBarrier* metric, 
                      MsqMatrix<Dim, Dim> A,
                      const MsqMatrix<Dim, Dim>& W,
                      double& value,
                      ...
                      MsqError& err )
{
  ...
  bool barrier_violated = false;
  bool valid = metric->evaluate_with_grad( A, W, value, grad, barrier_violated, err );
  if (MSQ_CHKERR(err) || !valid)
    return false;
  ...
  for (unsigned r = 0; r < Dim; ++r) {  // for each row of A
    for (unsigned c = 0; c < Dim; ++c) {  // for each column of A
      ...
      for (step = INITAL_STEP; step > std::numeric_limits<double>::epsilon(); step *= 0.1) {
        A(r,c) = in_val + step;
        valid = metric->evaluate_with_grad( A, W, value2, grad2, barrier_violated, err );  MSQ_ERRZERO(err);
        if (valid)
          break;
      }
...
bool AWMetricBarrier::evaluate( const MsqMatrix<2,2>& A, 
                                const MsqMatrix<2,2>& W,
                                double& result, 
                                bool barrier_violated,
                                MsqError& err )
{
    // this version of evaluate_with_grad() not valid at this level
  barrier_violated = false;
  return false;
}

bool AWMetricBarrier::evaluate( const MsqMatrix<3,3>& A, 
...
bool AWMetricBarrier::evaluate_with_grad( const MsqMatrix<2,2>& A,
                                          const MsqMatrix<2,2>& W,
                                          double& result,
                                          MsqMatrix<2,2>& wrt_A,
                                          bool barrier_violated,
...
bool AWMetricBarrier::evaluate_with_grad( const MsqMatrix<3,3>& A,
                                  const MsqMatrix<3,3>& W,
                                  double& result,
                                  MsqMatrix<3,3>& wrt_A,
                                  bool barrier_violated,
...
bool AWMetricBarrier::evaluate_with_hess( const MsqMatrix<2,2>& A,
                                   const MsqMatrix<2,2>& W,
                                   double& result,
...
bool AWMetricBarrier::evaluate_with_hess( const MsqMatrix<3,3>& A,
                                   const MsqMatrix<3,3>& W,
                                   double& result,
...
bool AWMetricBarrier2D::evaluate( const MsqMatrix<3,3>&, const MsqMatrix<3,3>&, double&, bool barrier_violated, MsqError& err )
{
...
bool AWMetricBarrier3D::evaluate( const MsqMatrix<2,2>&, const MsqMatrix<2,2>&, double&, bool barrier_violated, MsqError& err )
{
  MSQ_SETERR(err)("2D target metric cannot be evaluated for volume elements",
                  MsqError::UNSUPPORTED_ELEMENT);
  return false;
