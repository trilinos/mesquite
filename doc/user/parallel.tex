...
Mesquite currently has only a Nash-game type parallel optimization.  For algorithms such as Laplacian smoothing that are always Nash-game type optimizations, optimization in parallel is essentially the same as in serial.  For other optimizations that do a global minimization of an explicitly defined objective function in serial (for example the \texttt{ShapeImrovementWrapper}), the parallel optimization will be a Nash-game type optimization where the interior vertices (those not on the partition boundaries) will be optimized as a group.  Each vertices on the partition boundary will then be optimized individually.  While a global optimization in serial will typically have only one outer iteration, it is generally desirable to use some other outer termination criteria for parallel optimizations so that the optimization can
...
For a copy of an element to be available on a processor, all of its vertices must also be available on that processor.  So for all elements for which copes exist on more than one processor, the vertices contained in those elements must be ghosted on at least one processor.  For example, copies of the yellow elements in Figure \ref{fig:parallel_mesh} exist on both the blue and the green processors.  All blue vertices in at least one yellow element must be ghosted on the green processor and all green vertices in at least one yellow element must be ghosted on the blue processor.  A copy of the grey element must exist on every processor.  Therefore each vertex in that element must be ghosted on both of the other two processors that do not own it.  
...
This section contains two different examples demonstrating the use of the \texttt{ShapeImprovementWrapper} in parallel.


\subsection{Example: Parallel Shape Improvement}
\label{sec:parallel-example-1}

This example uses the \texttt{ShapeImprovementWrapper} in parallel using the built-in \texttt{Mesh}, \texttt{ParallelMesh}, and \texttt{ParallelHelperImpl} implementations.  For this example to work, the mesh must be partitioned such that the mesh for each processor is saved in a separate file named \texttt{part-\%d.vtk}, with the \texttt{\%d} replaced with the processor rank.  Each VTK file must contain vertex attributes named \texttt{GID} and \texttt{PID} containing the global ID and owning processor rank for each vertex.  Further, as this example provides no geometric domain definition, the vertices
on the boundary of the mesh must be fixed for the problem setup to be valid.
...
int main( int argc, char* argv[] )
{
  ...
  /* run shape-improvement wrapper */
  ShapeImprovementWrapper optimizer;
...
Similar to the example in Section \ref{sec:parallel-example-1}, this example uses the \texttt{ShapeImprovementWrapper} in parallel to improve element shape.  However, this example assumes that either the iMeshP implementation is partitioning or that it is reading some pre-defined partitioned mesh and it relies on the iMeshP implementation to create ghost elements, assign global vertex IDs, etc.  
...
int main( int argc, char* argv[] )
{
  ...
  /* create a partition instance in which to read the partitioned mesh */
  iMeshP_PartitionHandle partition;
  iMeshP_createPartitionAll(mesh, MPI_COMM_WORLD, &partition, &err);
  if (iBase_SUCCESS != ierr) return ierr;
