...
discretization means that the discretization error will assymptotically 
...
in time (such as in ALE simulatins) usually require that initally good 
...
the adaptive procedure. For example, if the initial mesh contains locally invertied elements, these can often be fixed before the adaptive procedure begins. As another example, if it is known \`{a} priori that small angles will be needed on the boundary of the domain to obtain reasonable simulation accuracy, one should try to first create the small angles in the initial mesh to improve the efficiency of the subsequent adaptive meshing proceedure. \newline


Many simulations, particluarly those in industry, are performed in a 
...
Another way to improve mesh quality is to use local topological modification methods in which mesh vertices or elements are locally created and/or destroyed. These methods are very successful when applied to simplicial meshes, often within an adaptive context.  Local topoology modification is less effective on nonsimplicial meshes. \newline

Mesh quality improvement remains an important on-going reserach area. 
...
algorithms are employed in Mesquite. Code interupts due to null pointers and 
...
non-adaptive simultion code, or as an algorithm for in-core adaptive mesh 
...
element in the mesh is decribed by a local quality metric that is a function 
...
solver when to halt. These are useful in controlling the tradeoff between
...
by settting the gradient of an objective function to zero. Such methods are
