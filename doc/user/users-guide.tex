...
\subsubsection{Compiling Mesquite} \label{sec:compiling}
...
\paragraph{Makefile Variables.}  \label{mes_vars_and_defs}
...
\subsubsection{Tutorial File Template}
\label{sec:tutfile}
...
\subsubsection{Loading a Test Mesh}
...
\begin{verbatim}
  ...
  my_mesh->read_vtk("../../meshFiles/2D/VTK/cube_hex_2.vtk", err); 
  MSQ_CHKERR(err);
\end{verbatim}
...
\end{verbatim}
Mesquite also provides a function to write a mesh file in VTK format, given a \texttt{MeshImpl}
object: 
\begin{verbatim}
...
\subsubsection{Improving the Mesh with a Wrapper Class}
...
\subsubsection{Improving the Mesh with the Low Level API}
...
\section{Writing a Mesquite Application}
...
\subsection{Simplified Application Programming Interface}
\label{sec:wrappers}
...
\subsection{Detailed Application Programming Interface}
